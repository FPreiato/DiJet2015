...
TFile file1("/cmshome/fpreiato/DiJet/test/CMSSW_7_2_1_DiJet/src/CMSDIJET/DijetRootTreeAnalyzer/SmearingFunctions_RecoGen_OneBinOneSampleAllEvents/rootfile_RSGravitonToQuarkQuark_kMpl01_M_3000_Tune4C_13TeV_pythia8__Phys14DR-PU20bx25_PHYS14_25_V1-v1__MINIAODSIM.root");
...
void analysisClass::Loop()
{
   ...
   /* see //  mi metto in un bin solo -> devo leggere un solo istogramma
   char HistoName[200];	 
   TH1D *h[100][100];
   ...
   */
   ...
   //   TH1D *h = new TH1D(HistoName, HistoName, 50,0.,2.);   ;
   //sprintf(HistoName,"Histo_RRecowide_Parton_%d_%d",ii,pt_bin_max);
   //h[ii][jj] = new TH1D(HistoName, HistoName, 50,0.,2.);   
   ///   
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    double GenDeltaEtaJJWide = 1000;
    double GenWide_cosThetaStar = 1000;    
    ...
    double GenDeltaR[2];   
    GenDeltaR[0] = Genwj1.DeltaR(parton1);
    GenDeltaR[1] = Genwj2.DeltaR(parton2);
    cout<< "DeltaR tra Genwidejet 1 e partone 1 = "<<GenDeltaR[0]<<endl;
    cout<< "DeltaR tra Genwidejet 2 e partone 2 = "<<GenDeltaR[1]<<endl;
    
    CreateAndFillUserTH1D("H_DeltaR_GenWideJet1_Parton1", 100, 0. , 3. , GenDeltaR[0]);
    CreateAndFillUserTH1D("H_DeltaR_GenWideJet2_Parton2", 100, 0. , 3. , GenDeltaR[1]);     
    ...
    /////////////////////////////////////////////////////////////////////////////
    // I plot dei widejet li riempio dopo cosi hanno lo stesso numero di eventi dei jets smeared
    ...
    double Eta_difference_Widejet1 = wj1.Eta() - parton1.Eta();
    double Phi_difference_Widejet1 = wj1.Phi() - parton1.Phi() ;
    double Eta_difference_Widejet2 = wj2.Eta() - parton2.Eta() ;
    double Phi_difference_Widejet2 = wj2.Phi() - parton2.Phi() ;
    
    double DeltaR[2];   
    DeltaR[0] = wj1.DeltaR(parton1);
    DeltaR[1] = wj2.DeltaR(parton2);
    cout<< "DeltaR tra widejet 1 e partone 1 = "<<DeltaR[0]<<endl;
    cout<< "DeltaR tra widejet 2 e partone 2 = "<<DeltaR[1]<<endl;
    
    CreateAndFillUserTH1D("H_Eta_difference_Widejet1", 100, -2. , 2. , Eta_difference_Widejet1);
    CreateAndFillUserTH1D("H_Phi_difference_Widejet1", 100, -2. , 2. , Phi_difference_Widejet1);
    CreateAndFillUserTH1D("H_Eta_difference_Widejet2", 100, -2. , 2. , Eta_difference_Widejet2);
    CreateAndFillUserTH1D("H_Phi_difference_Widejet2", 100, -2. , 2. , Phi_difference_Widejet2);       
    CreateAndFillUserTH1D("H_DeltaR_WideJet1_Parton1", 100, 0. , 3. , DeltaR[0]);
    CreateAndFillUserTH1D("H_DeltaR_WideJet2_Parton2", 100, 0. , 3. , DeltaR[1]);
    ...
    // I plot del sistema dijet li riempio dopo cosi hanno lo stesso numero di eventi dei jets smeared
    ...
    /////////////////////////////////////////////////////////////
    // mi calcolo il widejet partendo dai GenWideJet con le funzioni di smearing
    ...
    for(int kk = 0 ; kk < 2 ; kk++){
      ...
      //see //mi fisso in un bin in |eta| solo per adesso
      //	for(int ii=0; ii<5; ii++){
      int ii = 0; // see -> un bin solo
	 ...
	 //see //mi fisso in un bin in pT solo per adesso
	 //	    for(int jj=0; jj<n_bin ; jj++){	 
    ...
    double Ratio = DijetSmeared.M() / wdijet.M() ;
    
    CreateAndFillUserTH1D("H_Ratio_Masses", 100, -1. , 5. , Ratio); 
    CreateAndFillUserTH2D("H_RatioVSMasses", 1000, 0, 10000, 100, -1. , 5., wdijet.M(), Ratio); 
    cout<<"Rapporto tra le due masse: "<<Ratio<<endl;
    ...
    //Salvo variabili dei jet che ho ricostruito con le funzioni di smearing  
