...
TFileDirectory::_cd (const string &subdir, bool createNeededDirectories) const
{
  ...
  if ( dir == 0 ) 
      ...
      if (! createNeededDirectories)
	{
	  ...
	  cout<< "directory " << fpath << " doesn't exist."<<endl;
	}
      if ( ! path_.empty() ) 
	{
	  ...
	  if ( dir == 0 )
	      ...
	      //                throw 
	      //  cms::Exception( "InvalidDirectory" ) 
	      cout<< "Can't change directory to path: " << path_<<endl;
	    }
	} else 
	{
  ...
  if ( ! ok )
    {
      //          throw 
      // cms::Exception( "InvalidDirectory" ) 
      cout<< "Can't change directory to path: " << fpath<<endl;
    }
  return dir;
}
			...
			const string &description) const
{
  ...
  if ( ! subdirPtr ) {
      //         throw 
      //  cms::Exception( "InvalidDirectory" ) 
      cout<< "Can't create directory " << dir_ << " in path: " << path_<<endl;
    }
  subdirPtr->cd();
  return subdirPtr;
}
...
TFileDirectory::_getObj (const string &objname, const string &subdir) const
{
  ...
  if ( ! objPtr)
      ...
      if (subdir.length())
	{
	  ...
	  cout << "Can not find object named " << objname
	       << " in subdir " << subdir<<endl;
	} else {
	//  throw
	//   cms::Exception ("ObjectNotFound")
	cout << "Can not find object named " << objname<<endl;
      }
    } // if nothing found
  return objPtr;
}
