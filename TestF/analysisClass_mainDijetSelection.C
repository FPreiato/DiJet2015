	   ...
	   for(int ii=0; ii<5; ii++){
		 ...
		 if(parton_Pt[kk] >=pt_bin_min && parton_Pt[kk] < pt_bin_max){ 
		   ...
		   if(kk==0){
		     ...
		     char ProfileName_Pt[200];
		     sprintf(ProfileName_Pt,"Profile_R1_Pt_%d_%d",ii,pt_bin_max);
		     char ProfileName_Eta[200];
		     sprintf(ProfileName_Eta,"Profile_R1_Eta_%d_%d",ii,pt_bin_max);

		     if (R_PtWidejet_Ptparton[0] == wj1.Pt() / parton1.Pt()){

		       CreateAndFillUserTProfile(ProfileName_Pt, 20, pt_bin_min, pt_bin_max, 0., 2., parton2.Pt(), R_PtWidejet_Ptparton[0]);
		       CreateAndFillUserTProfile(ProfileName_Pt, 20, eta_bin_min, eta_bin_max, 0., 2., parton2.Eta(), R_PtWidejet_Ptparton[0]);
		     }
		     else if(R_PtWidejet_Ptparton[0] == wj2.Pt() / parton1.Pt()){
		       CreateAndFillUserTProfile(ProfileName_Eta, 20, pt_bin_min, pt_bin_max, 0., 2., parton1.Pt(), R_PtWidejet_Ptparton[0]);
		       CreateAndFillUserTProfile(ProfileName_Eta, 20, eta_bin_min, eta_bin_max, 0., 2., parton1.Eta(), R_PtWidejet_Ptparton[0]);
		     }
		   }
