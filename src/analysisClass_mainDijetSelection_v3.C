   ...
   // Create dijet(reco) system
    ...
    // Creo, riempio e salvo gli istogrammi     
     ...
     if(DeltaR_Widejet_parton1 <= 0.3){
       ...
       CreateAndFillUserTH1D("H_R_PtWidejet_PtParton1", 50, 0. , 2. , R_PtWidejet_PtParton1 );
	 ...
	 if( fabs(parton1.Eta())>=eta_bin_min && fabs(parton1.Eta()) < eta_bin_max){
	   ...
	   for(int jj=0; jj<n_bin ; jj++){	 
	     ...
	     if(parton1.Pt() >=pt_bin_min && parton1.Pt() < pt_bin_max){ 
	       
	       //cout<<"Bin in pT passed"<<endl;	       
	       
	       sprintf(HistoName,"Histo_RRecowide_Parton_%d_%d",ii,pt_bin_max);
	       CreateAndFillUserTH1D( HistoName, 50, 0. , 2. , R_PtWidejet_PtParton1 );
     ...
     if(DeltaR1_Widejet_GenWidejet1 < DeltaR2_Widejet_GenWidejet1){
       ...
       R_PtWidejet_PtGenWidejet1 = wj2.Pt() / Genwj2.Pt() ;
     }
     ...
     if(DeltaR_Widejet_GenWidejet1 <= 0.3){
       ...
       CreateAndFillUserTH1D("H_R_PtWidejet_PtGenWidejet1", 50, 0. , 2. , R_PtWidejet_PtGenWidejet1 );
       CreateAndFillUserTProfile("Profile_PtRecoWide", 1000, 0. , 10000. , 0., 3., Genwj1.Pt(), R_PtWidejet_PtGenWidejet1 );  
	 ...
	 if( fabs(Genwj1.Eta())>=eta_bin_min && fabs(Genwj1.Eta()) < eta_bin_max){
	   
	   for(int jj=0; jj<9 ; jj++){	 
	     
	     int pt_bin_min = ( (jj-1)*500)+500;
	     int pt_bin_max = (jj*500)+500;
	     
	     if(Genwj1.Pt() >=pt_bin_min && Genwj1.Pt() < pt_bin_max){ 
	       
	       sprintf(HistoName,"Histo_RRecowide_GenWidejet_%d_%d",ii,pt_bin_max);
	       CreateAndFillUserTH1D( HistoName, 50, 0. , 5. , R_PtWidejet_PtGenWidejet1 );
